...
class Database():
    ...
    def __init__(self):
        self.connection = sqlite3.connect(r'/Users/viktoriapakhomova/QA-Auto-2023' + r'/become_qa_auto.db')
    ...
    def update_customer_address_by_id(self, address, id):
        query = f"UPDATE customers SET address = '{address}' WHERE id = {id}"
        self.cursor.execute(query)
        self.connection.commit()


    def get_customer_address_by_id(self, id):
        query = f"SELECT address FROM customers WHERE id = {id}"
        self.cursor.execute(query)
        record = self.cursor.fetchall()
